...
class Parser:
    ...
    def __parse_html(self, html: str) -> list:
        ...
        **html:** str
        ...
        #print(pictures_place)
        urls = []
        try:
            pictures = pictures_place.find_all("div", {"class": "SerpItem"})
            #print(pictures)
            for pic in pictures:
                a = pic.find_all("a", {"class": "Link SimpleImage-Cover"})
                results = [iso['href'] for iso in a]
                image = f"{results[0]}"
                if ".jpg" in image:
                    image = image.partition(".jpg")[0]
                    image += "".join(".jpg")
                if ".png" in image:
                    image = image.partition(".png")[0]
                    image += "".join(".png")
                if ".jpeg" in image:
                    image = image.partition(".jpeg")[0]
                    image += "".join(".jpeg")
                image = urllib.parse.unquote(image)
                if "g&rpt=imageview" in image:
                    image = image.split("g&rpt=imageview")[0]
                    image += "".join("g")
                try:
                    image = image.split("_url=")[1]
                except Exception as e:
                    try:
                        image = image.split('=url:"')[1]
                    except Exception as e:
                        image = image.split('=url:"')
                        pass
                    #time.sleep(5)
                #print("https" in image)
